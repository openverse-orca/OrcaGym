...
def main():
    ...
    scripmonitor = subprocess.Popen(["python", "./device_client/client.py", 
        "--configfile", f"{current_file_path}/device_client/client_config.json",
        "--monitorpath", augmented_path
    ...
    try:
        ...
        print("\n中断接收，正在终止子进程...")
        # 向子进程发送终止信号
        scripmonitor.terminate()
        # 给子进程一些时间来清理
        try:
            scripmonitor.wait(timeout=5)
        except subprocess.TimeoutExpired:
            # 如果子进程没有在规定时间内终止，强制终止
            scripmonitor.kill()
            print("子进程被强制终止")
