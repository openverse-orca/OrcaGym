...
class ImageSender(Node):
    def __init__(self, port:int = 15631, ip:str="localhost"):
        super().__init__('img_send')
        self.subscription = self.create_subscription(
            Image,
            '/camera/camera_image_color',  # 替换为你的图像话题
            self.image_callback,
            20)
        self.subscription = self.create_subscription(
            Image,
            '/camera/camera_image_depth',  # 替换为你的图像话题
            self.depth_callback,
            20)
        ...
        self.url = "http://"+ip+f":{port}/ros2tf"  # 目标URL
        self.cnt = 0
...
def main(args=None):
    ...
    # node = ImageViewer()
    node = ImageSender(ip="192.168.110.135")
    # node = VideoPublisher()
