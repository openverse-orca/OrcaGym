    ...
    * v0: Initial versions release
    ...
    def __init__(
        ...
        **kwargs,
        ...
        cfrc_ext = self.loop.run_until_complete(self._query_all_cfrc_ext())
        # cfrc_ext 是一个 字典， key 是 body_name， value 是 cfrc_ext数组
        # 现在我们剔除 torso 的 cfrc_ext
        cfrc_ext.pop(self.body("torso"))
        cfrc_ext_size = sum([len(v) for v in cfrc_ext.values()]) * include_cfrc_ext_in_observation
        obs_size += cfrc_ext_size
    ...
    def contact_forces(self):
        cfrc_ext = self.loop.run_until_complete(self._query_all_cfrc_ext())
        cfrc_ext_value = np.array(list(cfrc_ext.values()))
        raw_contact_forces = cfrc_ext_value.flatten()
