...
class Acados_NMPC_Collaborative:
    def __init__(self):
        ...
        self.centroidal_model = Centroidal_Model_Collaborative()
    ...
    def create_ocp_solver_description(self, acados_model) -> AcadosOcp:
        ...
        init_end_effector_position = np.array([1, 1, 1])
        init_end_effector_jacobian_inv = np.array([1, 1, 1, 1, 1, 1, 1, 1, 1])
        init_end_effector_jacobian_transpose_inv = np.array([1, 1, 1, 1, 1, 1, 1, 1, 1])
        init_end_effector_gain = np.array([10, 10, 10])
        ocp.parameter_values = np.concatenate((init_contact_status, init_mu, init_stance_proximity, 
                                               ...
                                               init_end_effector_position, init_end_effector_jacobian_inv, 
                                               init_end_effector_jacobian_transpose_inv, init_end_effector_gain))
    ...
    def create_stability_constraints(self,) -> None: 
        ...
        if(self.use_static_stability):
            ...
            zmp_x = self.centroidal_model.mass*-gravity[2]*base_w[0] - base_w[2]*self.centroidal_model.mass*linear_com_acc[0]*0
            ...
            zmp_y = self.centroidal_model.mass*-gravity[2]*base_w[1] - base_w[2]*self.centroidal_model.mass*linear_com_acc[1]*0
    ...
    def set_weight(self, nx, nu):
        ...
        Q_base_angle = np.array([1000, 1200, 0])  #roll, pitch, yaw
    ...
    def compute_control(self, state, reference, contact_sequence, constraint = None, external_wrenches = np.zeros((6,)),
                        ...
                        end_effector_jacobian = np.ones((9,)), 
                        end_effector_gain = np.zeros((3,))):
        ...
        desired_force = np.array([0, 0]) #x, y, to be put in the reference
        ...
        for j in range(self.horizon):
            ...
            # If and Else to check if the reference is a single value or a sequence
            if(reference['ref_position'].shape[0] == 3):
        ...
        if(reference['ref_position'].shape[0] == 3):
            ...
            yref_N[0:3] = reference['ref_position'][-1]
        
        if(reference['ref_linear_velocity'].shape[0] == 3):
            yref_N[3:6] = reference['ref_linear_velocity']
        else:
            yref_N[3:6] = reference['ref_linear_velocity'][-1]
        
        if(reference['ref_orientation'].shape[0] == 3):
            yref_N[6:9] = reference['ref_orientation']
        else:
            yref_N[6:9] = reference['ref_orientation'][-1]
        
        if(reference['ref_angular_velocity'].shape[0] == 3):
            yref_N[9:12] = reference['ref_angular_velocity']
        else:
            yref_N[9:12] = reference['ref_angular_velocity'][-1]
        ...
        yref_N[30:32] = np.array([desired_force[0], desired_force[1]])
        # Setting the reference to acados
        ...
        for j in range(self.horizon):
            ...
            # To be filled with an estimation of the ee position in the WORLD
            end_effector_position = end_effector_position.reshape((3,))
            end_effector_jacobian = end_effector_jacobian.reshape((3, 3))
            
            end_effector_jacobian_inv = np.linalg.pinv(end_effector_jacobian)
            end_effector_jacobian_inv = end_effector_jacobian.reshape((9,))
            
            end_effector_jacobian_transpose_inv = np.linalg.pinv(end_effector_jacobian.T)
            end_effector_jacobian_transpose_inv = end_effector_jacobian_transpose_inv.reshape((9,))
            
            end_effector_gain = end_effector_gain.reshape((3,))
            ...
            param = np.array([FL_contact_sequence[j], FR_contact_sequence[j], 
                            ...
                            end_effector_jacobian_inv[0], end_effector_jacobian_inv[1], end_effector_jacobian_inv[2],
                            end_effector_jacobian_inv[3], end_effector_jacobian_inv[4], end_effector_jacobian_inv[5],
                            end_effector_jacobian_inv[6], end_effector_jacobian_inv[7], end_effector_jacobian_inv[8],
                            end_effector_jacobian_transpose_inv[0], end_effector_jacobian_transpose_inv[1], end_effector_jacobian_transpose_inv[2],
                            end_effector_jacobian_transpose_inv[3], end_effector_jacobian_transpose_inv[4], end_effector_jacobian_transpose_inv[5],
                            end_effector_jacobian_transpose_inv[6], end_effector_jacobian_transpose_inv[7], end_effector_jacobian_transpose_inv[8],
                            end_effector_gain[0], end_effector_gain[1], end_effector_gain[2]])
        ...
        state_acados = np.concatenate((state["position"], state["linear_velocity"],
                                ...
                                state["passive_arm_force"])).reshape((self.states_dim, 1))
