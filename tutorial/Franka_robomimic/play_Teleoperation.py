...
def register_env(env_name, env_index, **kwargs) -> str:
    orcagym_addr = kwargs['orcagym_addr']
...
def run_episode(env, dataset_writer):
    ...
    while True:
        ...
        ts_list.append(int(start_time.timestamp() * 1000))
        ...
        if terminated_times >= 5 or truncated:
            return obs_list, reward_list, done_list, info_list, ts_list
    ...
    while True:
        ...
        if save_camera:
            for camera_name in camera_name_list:
                camera = CameraCacher(camera_name, 7070)
                camera.start()
        ...
        obs_list, reward_list, done_list, info_list, ts_list = run_episode(env, dataset_writer)
        ...
        if save_record:
            ...
            dataset_writer.add_demo({
                ...
                'obs': obs_list,
                }, save_camera, camera_name_list, ts_list)
...
async def run_example():
    try:
        ...
        kwargs = {'frame_skip': 1,   
                    ...
                    'control_freq': 20}        
        env_id = register_env(env_name, env_index, **kwargs)
        ...
        dataset_writer = DatasetWriter(file_path=f"teleoperation_dataset.hdf5",
                                       env_name=env_id,
                                       env_version=env.get_env_version(),
                                       env_kwargs=kwargs)
