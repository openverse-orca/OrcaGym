...
│                                          │
│Input (observation):                      │
│  - Joint angles (query by name)          │
│  - End-effector position (query by name) │
│  - Target position                       │
│                                          │
│Output (action):                          │
│  - Target angle delta per joint          │
│                                          │
│Success condition:                        │
│  - End-effector to target < 5mm          │
│                                          │
│Internal controller:                      │
...
- Query state by name (query_joint_qpos / query_site_pos_and_mat)
- Set joints by name (set_joint_qpos / set_joint_qvel)
- PD controller
...
# ── Names needed in the scene (from the XML) ──────────────────────
EE_SITE_NAME = "end_effector"          # end-effector site name (passed to self.site(), which adds the agent prefix automatically)
# Joint name list (from the XML <joint> tags)
JOINT_NAMES = [
    "robot_0_shoulder_pan",
    "robot_0_shoulder_lift",
    "robot_0_elbow",
    "robot_0_wrist_1",
    "robot_0_wrist_2",
    "robot_0_wrist_3",
...
class ReachTargetTask(OrcaGymEulerEnv):
    ...
    (query by name, PD control, reward computation) into a complete working task.
    """
    ...
    def __init__(self, frame_skip, orcagym_addr, agent_names, time_step, **kwargs):
        ...
        # ── PD controller ──
        self._pd = PDController(nu=NU, kp=150.0, kd=12.0)
        ...
        print(f"ReachTargetTask ready: nu={NU}, "
              f"dt={self.dt:.4f}s, control frequency={1.0/self.dt:.1f}Hz")

    # ================================================================
    # Observation (all queried by name)
    # ================================================================
    def _get_obs(self) -> dict:
        """Collect observation: joint state (by name) + end-effector pose (by name) + target position"""
        # Query joint positions and velocities by name
        qpos_dict = self.query_joint_qpos(JOINT_NAMES)
        qvel_dict = self.query_joint_qvel(JOINT_NAMES)
        # Concatenate into arrays in name order
        joint_pos = np.array([qpos_dict[n][0] for n in JOINT_NAMES], dtype=np.float32)
        joint_vel = np.array([qvel_dict[n][0] for n in JOINT_NAMES], dtype=np.float32)

        # Query the end-effector site pose by name (self.site() adds the agent prefix automatically)
        ee_site = self.site(EE_SITE_NAME)
    ...
    def step(self, action: np.ndarray):
        ...
        # 2. Query current joint positions and velocities by name
        qpos_dict = self.query_joint_qpos(JOINT_NAMES)
        qvel_dict = self.query_joint_qvel(JOINT_NAMES)
        current_qpos = np.array([qpos_dict[n][0] for n in JOINT_NAMES])
        current_qvel = np.array([qvel_dict[n][0] for n in JOINT_NAMES])

        # 3. Target position = current position + delta
        target_qpos = current_qpos + action

        # 4. PD controller → torque → simulation step
        ctrl = self._pd.compute(target_qpos, current_qpos, current_qvel)
    ...
    def reset_model(self):
        ...
        # Randomly sample target position within the workspace
...
if ENV_ID not in gym.envs.registry:
    gym.register(
        ...
        kwargs={
            ...
            'agent_names': ["robot_0"],
            'time_step': 0.001,
            # model_xml_path passes the local XML path via kwargs (required for offline mode)
...
def manual_control_demo():
    ...
    This demonstrates how to complete a simulation task with manual rules, without using RL.
    """
    print("=" * 60)
    ...
    for step_idx in range(300):
        ...
        action = np.random.randn(NU) * 0.02
...
│
├── step(action)
│   ├── query_joint_qpos(names)     ← query current joint position by name
│   ├── query_joint_qvel(names)     ← query current joint velocity by name
│   ├── target = current + action   ← parse action
│   ├── ctrl = pd.compute(...)      ← PD → torque
│   ├── do_simulation(ctrl, ...)    ← physics simulation
│   ├── obs = _get_obs()            ← collect observation
│   └── _compute_reward(obs, ...)   ← compute reward
│
└── _get_obs()
    ├── query_joint_qpos(names)          ← query joint position by name
    ├── query_joint_qvel(names)          ← query joint velocity by name
    ├── query_site_pos_and_mat(names)    ← query end-effector pose by name
...
| Reading State | Query joints/bodies/sites by name | `query_joint_qpos()`, `query_site_pos_and_mat()` |
| Controlling Joints | Set qpos/qvel by name | `set_joint_qpos()`, `set_joint_qvel()` |
