# 🐋 Overview

Welcome to the **OrcaGym** documentation!

OrcaGym is an open-source robot simulation platform that provides an interface fully compatible with OpenAI Gym/Gymnasium, supporting multiple physics backends and distributed simulation.

---

## Documentation Navigation

| Section | Description |
|---------|-------------|
| [🐋 What is OrcaGym](what-is-orcagym.md) | What OrcaGym is, core features at a glance |
| [🧬 Why Choose OrcaGym](why-orcagym.md) | Comparison and advantages over other simulation platforms |
| [🛠️ Installation Guide](installation.md) | Installation and configuration |

---

## Quick Overview

OrcaGym is a **cloud-native robot simulation platform** with the following core features:

- :material-gym: **Gymnasium Compatible** — Seamless integration with existing RL algorithms
- :material-engine: **Multi-Physics Backend** — Locally supports MuJoCo/MuJoCoWarp/Euler; PhysX and ODE can be accessed via OrcaStudio/OrcaLab
- :material-cloud: **Distributed Deployment** — Supports local development and remote large-scale training
- :material-camera: **Ray Tracing Rendering** — Realistic visual observations provided by OrcaStudio/OrcaLab
- :material-robot: **Multi-Agent Support** — Native heterogeneous agent management

---

## Who Should Use OrcaGym?

- **Reinforcement Learning Researchers** — Need high-performance, standardized Gymnasium environments for training policies
- **Robotics Developers** — Need workflows integrated with the OrcaStudio and OrcaLab platforms
- **Simulation Engineers** — Need the flexibility of multiple physics backends and distributed deployment capabilities
- **Educators** — Need an easy-to-use robot simulation teaching platform

---

## Companion Products

OrcaGym is part of the robot simulation ecosystem:

| Product | Description |
|---------|-------------|
| **OrcaStudio** | Visual scene editing and simulation management tool |
| **OrcaLab** | Cloud-based robot training and evaluation platform |
| **OrcaGym** (this repo) | Python simulation interface library |

---

## Next Steps

Start with the [Installation Guide](installation.md), then read the [Beginner's Tutorial](../tutorial/beginners/index.md) to run your first simulation!
