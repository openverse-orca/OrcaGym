...
整合了前面教程的所有知识：
- 环境子类化
- 按名称查询状态（query_joint_qpos / query_site_pos_and_mat）
- 按名称设置关节（set_joint_qpos / set_joint_qvel）
- PD 控制器
...
# ── 场景中需要用到的名称（从 XML 中获取） ──────────────────────
EE_SITE_NAME = "end_effector"          # 末端执行器 site 名称（传给 self.site() 会自动加 agent 前缀）
# 关节名称列表（从 XML 的 <joint> 标签中获取）
JOINT_NAMES = [
...
class ReachTargetTask(OrcaGymEulerEnv):
    ...
    这个环境展示了如何将多个组件（按名称查询、PD控制、奖励计算）
    组合成一个完整的工作任务。
    """
    ...
    def __init__(self, frame_skip, orcagym_addr, agent_names, time_step, **kwargs):
        ...
        print(f"ReachTargetTask 就绪: nu={NU}, "
    ...
    def _get_obs(self) -> dict:
        """收集观测：关节状态（按名称查询）+ 末端位姿（按名称查询）+ 目标位置"""
        # 按名称查询关节位置和速度
        ...
        # 按名称查询末端 site 位姿（self.site() 自动加 agent 前缀）
...
│
├── step(action)
│   ├── query_joint_qpos(names)     ← 按名称查询当前关节位置
│   ├── query_joint_qvel(names)     ← 按名称查询当前关节速度
│   ├── target = current + action   ← 解析动作
│   ├── ctrl = pd.compute(...)      ← PD → 力矩
│   ├── do_simulation(ctrl, ...)    ← 物理仿真
│   ├── obs = _get_obs()            ← 收集观测
│   └── _compute_reward(obs, ...)   ← 计算奖励
│
└── _get_obs()
    ├── query_joint_qpos(names)          ← 按名称查询关节位置
    ├── query_joint_qvel(names)          ← 按名称查询关节速度
    ├── query_site_pos_and_mat(names)    ← 按名称查询末端位姿
    └── 计算到目标的距离
```
...
| 读取状态 | 按名称查询关节/Body/Site | `query_joint_qpos()`、`query_site_pos_and_mat()` |
| 控制关节 | 按名称设置 qpos/qvel | `set_joint_qpos()`、`set_joint_qvel()` |
