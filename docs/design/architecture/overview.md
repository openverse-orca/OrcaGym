...
本文面向 Orca 体系的开发者和用户，从分层视角描述 `OrcaGymEulerEnv` + `OrcaGymEuler` 系统的整体架构，厘清各层职责与 API 边界，帮助开发者判断"应在哪一层开发"以及"应维护哪一层"。

组件设计细节、API 契约、封装隔离机制见 [orca_gym_euler_architecture.md](orca_gym_euler_architecture.md)。
...
详细契约与隔离机制见 [orca_gym_euler_architecture.md](orca_gym_euler_architecture.md) §6–§7。
